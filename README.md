...
| `dcp.group.membership.memberNumber`      |        int        |    no    |     1      | Set this if membership is `static`. Other methods will ignore this field. Setting `GO_DCP_DCP_GROUP_MEMBERSHIP_MEMBERNUMBER` as an environment variable will overwrite this config, to be able to prevent making deployment to scale up or down.                            |
| `dcp.group.membership.totalMembers`      |        int        |    no    |     1      | Set this if membership is `static` or `kubernetesStatefulSet`. Other methods will ignore this field. Setting `GO_DCP_DCP_GROUP_MEMBERSHIP_TOTALMEMBERS` as an environment variable will overwrite this config, to be able to prevent making deployment to scale up or down. |

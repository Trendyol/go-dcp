...
| `dcp.group.membership.memberNumber`      |        int        |    no    |     1      | Set this if membership is `static`. Other methods will ignore this field. Setting `memberNumber` as an environment variable will overwrite this config, to be able to prevent making deployment to scale up or down.                            |
| `dcp.group.membership.totalMembers`      |        int        |    no    |     1      | Set this if membership is `static` or `kubernetesStatefulSet`. Other methods will ignore this field. Setting `totalMembers` as an environment variable will overwrite this config, to be able to prevent making deployment to scale up or down. |
